...
void loading(){
	...
	outtextxy(220,235,"Loading....");
	rectangle(190,275,420,295);
	int i=191,j=275,i1=192,j1=295,x=1,y=0;
	setcolor(BLACK);
	while (i<=419){
		delay(1);
		rectangle(i,j,i1,j1);
...
int main (){
	...
	while(obs){
	    	...
	    	j1=j1+15;
	    	for(int a=0; a<=3; a++){
	        	bar(x[a]+flow, 0, x[a]+60+flow, h[a] );
	        	bar (x[a]+flow, h[a]+250, x[a]+60+flow, getmaxy());
	    	    if (x[a]+150+flow<0){
	    	    	x[a]=getmaxx()-flow;
				}
	    		if (x[a]+10+flow<i && x[a]+25+flow>i){
	    			score++;
				}
				if(x[a]+flow<i+30 && x[a]+flow+60>i+30 && 0<j-30 && h[a]>j-30){
					obs=false;
				} 
				if(x[a]+flow<i+30 && x[a]+flow+60>i+30 && h[a]+250<j-30 && getmaxy()>j-30){
					obs=false;
				} 
			}
			star();
